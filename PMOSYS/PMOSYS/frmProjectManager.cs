    ...
    public partial class frmProjectManager : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = @"INSERT INTO  [ProjectManagers]
                                           ([ProNo]
                                           ,[ProNameE]
                                           ,[ProNameA]
                                           ,[ProUserID]
        ...
        void GetAllData()
        {
            ...
            da = new SqlDataAdapter(@"select * from ProjectManagers", ConCls.con);
            dt = new DataTable();
            ConCls.OpenConnection();
            da.Fill(dt);
            ConCls.CloseConnection();
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                sql_str = @"select * from ProjectManagers where ProNo like '%" + txtSearch.Text + "%'";
            }

            else if (comboBox2.SelectedIndex == 1) // full name english 
            {
                sql_str = @"select * from ProjectManagers where ProNameE like '%" + txtSearch.Text + "%'";
            }
            else if (comboBox2.SelectedIndex == 2) // full name arabic
            {
                sql_str = @"select * from ProjectManagers where ProNameA like '%" + txtSearch.Text + "%'";
            }
            else if (comboBox2.SelectedIndex == 3) // email account
            {
                sql_str = @"select * from ProjectManagers where ProEmail  like '%" + txtSearch.Text + "%'";
            }
            else if (comboBox2.SelectedIndex == 4) // work phone 1
            {
                sql_str = @"select * from ProjectManagers where ProWorkPhone1 like '%" + txtSearch.Text + "%'";
            }
            else if (comboBox2.SelectedIndex == 5) // mob phone 1
            {
                sql_str = @"select * from ProjectManagers where ProMobPhone1 like '%" + txtSearch.Text + "%'";
            }
        ...
        {
            //string empNo = dataGridView1.CurrentRow.Cells[].Value.ToString();
            //string empN = dataGridView1.CurrentRow.Cells[].Value.ToString();
        }
    }
}
